...
def recommend_careers(user_skills, job_df, top_n=3):
    ...
    # Combine job roles + user into one shared vocabulary space
    all_texts = job_df["skills_text"].tolist() + [user_text]

    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform(all_texts)

    # VOCABULARY-MISMATCH CHECK: see which of the user's words the
    # vectorizer actually recognized from the shared skill vocabulary.
    learned_vocab = set(vectorizer.get_feature_names_out())
    user_words = set(user_text.lower().split())
    recognized_words = user_words & learned_vocab
    unrecognized_words = user_words - learned_vocab
